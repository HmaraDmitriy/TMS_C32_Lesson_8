...
public class VisaCardTransferService implements CardTransferService {
    ...
    public Check transferFromCardToCard(BaseCard senderCard, BaseCard receiverCard, double sum, int data, int numberCart) {
        if (senderCard.checkCardLimitTransfer(sum)) {
            ...
            System.out.println("Transfer from VisaCard to another card completed successfully.");
            return new Check(sum, data, numberCart);
        } else {
            System.out.println("Transfer from VisaCard to another card failed.");
            return new Check(sum, data, numberCart);
        }
    }
    ...
    public Check transferFromCardToAccount(BaseCard senderCard, Account receiverAccount, double sum, int data, int numberCart) {
        if (senderCard.checkCardLimitTransfer(sum)) {
            ...
            System.out.println("Transfer from VisaCard to account completed successfully.");
            return new Check(sum, data, numberCart);
        } else {
            System.out.println("Transfer from VisaCard to account failed.");
            return new Check(sum, data, numberCart);
        }
    }
}
